...
# NOUVEAU SYSTÈME DE GESTION DES ABONNEMENTS
utilisateurs_autorises = {ADMIN_ID: "LIFETIME"} # Dictionnaire {ID: Expiration}
cles_generees = {} # Stockage ultra-sécurisé {Clé: {"jours": X, "user_id": Y}}
...
def est_autorise(user_id):
    ...
    if user_id in utilisateurs_autorises:
        ...
        if datetime.datetime.now() < expiration:
            ...
            # L'abonnement est expiré, on le supprime
...
def activer_cle(message):
    ...
    if cle in cles_generees:
        ...
        # VÉRIFICATION DE SÉCURITÉ : La clé appartient-elle bien à CET utilisateur ?
        ...
        del cles_generees[cle] # Détruit la clé pour qu'elle ne serve qu'une fois
...
def creer_cle(call):
    ...
    # ON ENREGISTRE LA CLÉ AVEC L'ID DE L'ACHETEUR POUR L'ANTI-PARTAGE
...
Bienvenue dans ton radar de trading ultime ! Moteur : Pandas + TA.

📖 **MODE D'EMPLOI :**
1️⃣ Clique sur "📊 CHOISIR UNE DEVISE".
2️⃣ Clique sur "🚀 LANCER L'ANALYSE" pour le verrouillage Sniper.
3️⃣ Consulte "⏰ HEURES DE TRADING" pour les meilleures fenêtres.

💡 **LE MOT DU FONDATEUR :**
*Le marché ne ressent rien, n'aie aucune émotion. Le succès vient d'une discipline de fer. Laisse l'algorithme faire les calculs et protège ton capital comme un tireur d'élite. Bon profit !* 🎯💸"""
    ...
    bot.send_message(message.chat.id, texte_bienvenue, reply_markup=markup, parse_mode="Markdown")
...
def vision_marche(message):
    ...
    msg = bot.send_message(message.chat.id, f"🔍 *Scan de {symbole}...*", parse_mode="Markdown")
    url = f"https://query1.finance.yahoo.com/v8/finance/chart/{symbole}=X?range=2d&interval=1m"
    try:
        donnees = requests.get(url, headers={'User-Agent': 'Mozilla/5.0'}, timeout=10).json()
        df = pd.DataFrame({'close': donnees['chart']['result'][0]['indicators']['quote'][0]['close']}).dropna()
        if len(df) < 50: bot.edit_message_text("⚠️ Manque de données.", message.chat.id, msg.message_id); return
        prix = df['close'].iloc[-1]
        bot.edit_message_text(f"👁️ **VISION RAYONS X : {symbole}** 👁️\n💰 **Prix :** `{prix:.5f}`\n💤 *Scan Rapide Actif.*", message.chat.id, msg.message_id, parse_mode="Markdown")
    except:
        bot.edit_message_text("❌ Erreur.", message.chat.id, msg.message_id)

# --- SÉCURITÉ SUR LES AUTRES BOUTONS ---
@bot.message_handler(func=lambda m: m.text == "⏰ HEURES DE TRADING")
def horaires_trading(message):
    if not est_autorise(message.chat.id): return
    texte = "🕒 **GUIDE DES HORAIRES (GMT)** 🕒\n\n✅ **MATIN (08h00-11h00):** EUR/USD, EUR/JPY, CHF/JPY\n🔥 **OR (13h30-16h30):** EUR/USD, USD/CAD, AUD/USD\n❌ **NUIT (22h00-07h00):** À FUIRE."
...
def devises(message):
    ...
    bot.send_message(message.chat.id, "Sélectionne l'actif :", reply_markup=markup)
...
def lancer(message):
    ...
    if not actif: bot.send_message(message.chat.id, "⚠️ Choisis d'abord une devise !"); return
    
    msg = bot.send_message(message.chat.id, "⏳ *Scan algorithmique...*", parse_mode="Markdown")
    action, confiance, exp, duree = analyser_binaire_pro(actif)
    
    if not action or "⚠️" in action:
        bot.edit_message_text("⚠️ Marché neutre. Patientez.", message.chat.id, msg.message_id)
        return

    maintenant = datetime.datetime.now()
    heure_entree = (maintenant + datetime.timedelta(minutes=2)).replace(second=0).strftime("%H:%M:00")
