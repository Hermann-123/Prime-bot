...
if message.chat.id != ADMIN_ID: return
if not utilisateurs_autorises: return bot.send_message(message.chat.id, "Aucun abonné actif.")
lignes = ["👥 ABONNÉS ACTIFS :\n──────────────────"]
now = datetime.datetime.now()
for uid, exp in utilisateurs_autorises.items():
if uid == ADMIN_ID: continue
if exp == "LIFETIME": statut = "👑 À vie"
elif now < exp: statut = f"✅ {(exp - now).days}j restants (exp: {exp.strftime('%d/%m/%Y')})"
else: statut = "❌ Expiré"
...
if message.chat.id != ADMIN_ID: return
if not cles_generees: return bot.send_message(message.chat.id, "Aucune clé en attente.")
...
if est_gold_ou_index_actif(): return "AUTORISE", ""
...
if s in mapping_specifique: return mapping_specifique[s]
...
diff = sh - sl
if tendance == "BEAR":
ob, oh = sl+diff0.618, sl+diff0.786
sl_lvl = sh + diff0.05
dist = abs(oh - sl_lvl)
tp1, tp15 = oh - dist, oh - dist1.5
else:
ob, oh = sh-diff0.786, sh-diff0.618
sl_lvl = sl - diff0.05
dist = abs(ob - sl_lvl)
tp1, tp15 = ob + dist, ob + dist1.5
...
if meche_h > corps2.0: return True,"📍 Pin Bar Baissier"
if last['close']<last['open'] and corps>taille0.4: return True,"📉 Rejet Baissier"
else:
if prev['close']<prev['open'] and last['close']>last['open'] and last['close']>prev['open']: return True,"🕯️ Engulfing Haussier"
if meche_b > corps2.0: return True,"📍 Pin Bar Haussier"
if last['close']>last['open'] and corps>taille0.4: return True,"📈 Rejet Haussier"
...
if now - t.get('ts', now) > t.get('dur',300)+120: del trades_en_cours[uid]
...
f"📍 {res['msg']}\n"
f"🔑 {res['kz']}\n"
f"🟡 Zone OTE : {z['ote_bas']} → {z['ote_haut']}\n"
f"⚖️ R/R : {res['rr']}R\n"
f"90s pour agir"
)
...
f"━━━━━━━━━━━━━━━━━━━━━━\n"
f"⏳ En attente d'une réaction structurelle dans la zone.\n"
f"⏰ Session : {kz}")
else:
texte = "⚠️ Données indisponibles. Réessaie dans quelques instants."
