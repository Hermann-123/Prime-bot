...
TELEGRAM_TOKEN = "8658287331:AAHrsllqegvSeB1_lk-0heiuyuuxYbtxJZ4"
...
# ============================================================
# MODULE ICT OTE — STRATÉGIE VIDÉO 2026
# EMA CLOUD (72/89/180/200) + FIBONACCI OTE (0.618-0.786)
# ============================================================

def calculer_ema_cloud(df):
    """
    Ripster EMA Clouds comme dans la vidéo :
    - EMA courte  : 72 et 89  → nuage rapide
    - EMA longue  : 180 et 200 → nuage lent (tendance de fond)
    Nuage vert (ema72 > ema89)  → tendance haussière
    Nuage rouge (ema72 < ema89) → tendance baissière
    """
    ...
    # Les deux nuages alignés = signal fort
    if nuage_rapide == "BULL" and nuage_lent == "BULL":
        tendance = "BULL"
        force    = "FORT 🟢🟢"
    elif nuage_rapide == "BEAR" and nuage_lent == "BEAR":
        tendance = "BEAR"
        force    = "FORT 🔴🔴"
    elif nuage_rapide == "BULL":
        tendance = "BULL"
        force    = "MODÉRÉ 🟡"
    else:
        tendance = "BEAR"
    ...
    return tendance, force, {
        "ema72": round(ema72.iloc[-1], 3),
        "ema89": round(ema89.iloc[-1], 3),
        "ema180": round(ema180.iloc[-1], 3),
        "ema200": round(ema200.iloc[-1], 3)
    }


def trouver_swing_recant(df, tendance, lookback=30):
    """
    Identifie le dernier swing High/Low significatif.
    En tendance BEAR  → on cherche le dernier swing High puis le Low suivant
    En tendance BULL  → on cherche le dernier swing Low puis le High suivant
    C'est le swing sur lequel on trace le Fibonacci.
    """
    highs = df['high'].iloc[-lookback:]
    lows  = df['low'].iloc[-lookback:]

    if tendance == "BEAR":
        # Swing : du haut récent vers le bas récent (retracement haussier = zone SELL)
        swing_high = highs.max()
        idx_high   = highs.idxmax()
        # Low après le swing high
        swing_low  = lows.iloc[lows.index.get_loc(idx_high):].min()
        return swing_high, swing_low, "HIGH_TO_LOW"
    else:
        # Swing : du bas récent vers le haut récent (retracement baissier = zone BUY)
        swing_low  = lows.min()
        idx_low    = lows.idxmin()
        swing_high = highs.iloc[highs.index.get_loc(idx_low):].max()
        return swing_high, swing_low, "LOW_TO_HIGH"


def calculer_zone_ote(swing_high, swing_low, tendance):
    """
    Zone OTE ICT = Fibonacci 0.618 à 0.786 du swing.
    
    En BEAR (retracement haussier avant continuation baissière) :
      → OTE zone = entre fib 0.618 et 0.786 depuis le BAS vers le HAUT
      → Le prix remonte dans cette zone = entrée SELL
    
    En BULL (retracement baissier avant continuation haussière) :
      → OTE zone = entre fib 0.618 et 0.786 depuis le HAUT vers le BAS
      → Le prix descend dans cette zone = entrée BUY
    
    SL = au-delà du niveau 1.0 (haut ou bas du swing)
    TP = niveau 0.0 du swing (début du mouvement) × 1.5R
    """
    diff = swing_high - swing_low

    if tendance == "BEAR":
        # Retracement haussier : on vend dans la zone
        ote_bas  = swing_low + diff * 0.618   # 61.8%
        ote_haut = swing_low + diff * 0.786   # 78.6%
        sl       = swing_high + diff * 0.1    # Légèrement au-dessus du swing high
        tp_1r    = swing_low                  # Retour au swing low
        tp_15r   = swing_low - diff * 0.5     # TP 1.5R
    else:
        # Retracement baissier : on achète dans la zone
        ote_bas  = swing_high - diff * 0.786  # 78.6% depuis le haut
        ote_haut = swing_high - diff * 0.618  # 61.8% depuis le haut
        sl       = swing_low - diff * 0.1     # Légèrement sous le swing low
        tp_1r    = swing_high                 # Retour au swing high
        tp_15r   = swing_high + diff * 0.5    # TP 1.5R

    return {
        "ote_bas":  round(ote_bas, 3),
        "ote_haut": round(ote_haut, 3),
        "sl":       round(sl, 3),
        "tp_1r":    round(tp_1r, 3),
        "tp_15r":   round(tp_15r, 3),
        "fib_0":    round(swing_low if tendance=="BEAR" else swing_high, 3),
        "fib_1":    round(swing_high if tendance=="BEAR" else swing_low, 3),
    }


def detecter_reaction_ote(df, zone_ote, tendance):
    """
    Étape 4 de la vidéo : attendre UNE RÉACTION dans la zone OTE.
    
    Signaux de réaction valides :
    - Mèche longue de rejet
    - Bougie engulfing
    - Pin bar
    """
    last = df.iloc[-1]
    ...
    # Le prix est-il dans la zone OTE ?
    dans_zone = zone_ote["ote_bas"] <= px <= zone_ote["ote_haut"]
    # Ou la bougie précédente était dans la zone ?
    prev_dans_zone = zone_ote["ote_bas"] <= prev['close'] <= zone_ote["ote_haut"]

    if not (dans_zone or prev_dans_zone):
        return False, "Prix hors zone OTE"
    ...
    if tendance == "BEAR":
        # On cherche une réaction baissière dans la zone
        engulfing_bear = prev['close'] > prev['open'] and last['close'] < last['open'] and \
                         last['close'] < prev['open'] and corps > 0
        pin_bar_bear   = meche_h > corps * 2.0 and taille > 0
        if engulfing_bear:
            return True, "🕯️ Engulfing Baissier en zone OTE"
        if pin_bar_bear:
            return True, "📍 Pin Bar Baissier en zone OTE"
        if last['close'] < last['open'] and corps > taille * 0.4:
            return True, "📉 Bougie baissière en zone OTE"

    else:  # BULL
        engulfing_bull = prev['close'] < prev['open'] and last['close'] > last['open'] and \
                         last['close'] > prev['open'] and corps > 0
        pin_bar_bull   = meche_b > corps * 2.0 and taille > 0
        if engulfing_bull:
            return True, "🕯️ Engulfing Haussier en zone OTE"
        if pin_bar_bull:
            return True, "📍 Pin Bar Haussier en zone OTE"
        if last['close'] > last['open'] and corps > taille * 0.4:
            return True, "📈 Bougie haussière en zone OTE"

    return False, "Pas de réaction en zone OTE"


def analyser_ote_complet(symbole):
    """
    Moteur complet ICT OTE — applique les 6 étapes de la vidéo.
    Fonctionne sur XAUUSD et toutes les paires Forex.
    Retourne un dict complet avec signal, zone, SL, TP.
    """
    # Données M5 pour l'OTE (comme dans la vidéo)
    candles_m5 = obtenir_donnees_deriv(symbole, 300)
    # Données H1 pour l'EMA Cloud (tendance de fond)
    ...
    try:
        df_m5 = pd.DataFrame([{'open':float(c['open']),'close':float(c['close']),
                                'high':float(c['high']),'low':float(c['low'])} for c in candles_m5])
        df_h1 = pd.DataFrame([{'open':float(c['open']),'close':float(c['close']),
                                'high':float(c['high']),'low':float(c['low'])} for c in candles_h1])

        # ── ÉTAPE 2 : EMA Cloud sur H1 ──────────────────────────────────
        tendance, force_cloud, emas = calculer_ema_cloud(df_h1)

        # ── ÉTAPE 3 : Fibonacci OTE sur M5 ──────────────────────────────
        swing_h, swing_l, type_swing = trouver_swing_recant(df_m5, tendance, lookback=40)
        diff = swing_h - swing_l

        # Swing trop petit = pas fiable
        if diff < 0.5:  # Moins de 50 pips sur Gold
            return None

        zone_ote = calculer_zone_ote(swing_h, swing_l, tendance)

        # ── ÉTAPE 4 : Réaction en zone OTE ──────────────────────────────
        reaction_ok, reaction_msg = detecter_reaction_ote(df_m5, zone_ote, tendance)

        px_actuel = df_m5['close'].iloc[-1]

        # Calcul R/R
        risque    = abs(px_actuel - zone_ote["sl"])
        recompense= abs(zone_ote["tp_15r"] - px_actuel)
        rr        = round(recompense / risque, 2) if risque > 0 else 0

        # Score global
        score = 0
        if tendance == ("BULL" if "BUY" in (tendance) else "BEAR"): score += 3
        score += 3  # EMA cloud
        if reaction_ok: score += 3
        if rr >= 1.5: score += 1
        score = min(score, 10)
        ...
        return {
            ...
            "emas":       emas,
            "zone_ote":   zone_ote,
            "swing_high": round(swing_h, 3),
            "swing_low":  round(swing_l, 3),
            "reaction":   reaction_ok,
            "reaction_msg": reaction_msg,
            "px_actuel":  round(px_actuel, 3),
            "rr":         rr,
            "score":      score,
            "signal":     ("🟢 BUY" if tendance=="BULL" else "🔴 SELL") if reaction_ok else None
        }
...
def formater_signal_ote(res, capital):
    """Formate le message Telegram du signal OTE comme dans la vidéo."""
    ...
    dir = res["tendance"]
    rr  = res["rr"]
    mise= round(capital * 0.01, 0)

    return f"""🎯 **ICT OTE SIGNAL — {res['symbole']}** 🎯
━━━━━━━━━━━━━━━━━━━━━━
{'🟢 **BUY**' if dir=='BULL' else '🔴 **SELL**'} | Score : `{res['score']}/10`
☁️ EMA Cloud : `{res['force']}`
━━━━━━━━━━━━━━━━━━━━━━
📍 **Swing High :** `{res['swing_high']}`
📍 **Swing Low  :** `{res['swing_low']}`
━━━━━━━━━━━━━━━━━━━━━━
🟡 **Zone OTE (0.618-0.786) :**
   `{z['ote_bas']}` → `{z['ote_haut']}`
💰 **Prix actuel :** `{res['px_actuel']}`
━━━━━━━━━━━━━━━━━━━━━━
🛑 **SL :** `{z['sl']}`
🎯 **TP 1R :** `{z['tp_1r']}`
🚀 **TP 1.5R :** `{z['tp_15r']}`
...
📋 *Méthode ICT OTE | EMA Cloud H1 + Fib M5*"""


# ==========================================
# COMMANDE /ote — Analyse OTE manuelle
# ==========================================
...
def cmd_ote(message):
    ...
    msg = bot.send_message(uid, f"🔍 *Analyse ICT OTE sur {symbole}...*", parse_mode="Markdown")
    res = analyser_ote_complet(symbole)
    if not res:
        return bot.edit_message_text("⚠️ Données insuffisantes pour l'OTE.", uid, msg.message_id)
    z = res["zone_ote"]
    if res.get("signal"):
        texte = formater_signal_ote(res, CAPITAL_ACTUEL)
    else:
        texte = f"""👁️ **OTE ANALYSE : {symbole}**
━━━━━━━━━━━━━━━━━━━━━━
☁️ EMA Cloud H1 : `{res['force']}` ({'🟢 BULL' if res['tendance']=='BULL' else '🔴 BEAR'})
🟡 **Zone OTE :** `{z['ote_bas']}` → `{z['ote_haut']}`
💰 **Prix actuel :** `{res['px_actuel']}`
⏳ **Statut :** En attente d'une réaction dans la zone
🛑 SL : `{z['sl']}` | 🎯 TP : `{z['tp_15r']}`"""
    bot.edit_message_text(texte, uid, msg.message_id, parse_mode="Markdown")


# ==========================================
# INTÉGRATION OTE DANS LE SCANNER AUTO
# ==========================================
# Ce bloc est appelé dans scanner_marche_auto()
# Remplace l'ancien bloc Gold pour inclure l'OTE

_ote_derniere_analyse = {}

def scanner_ote_toutes_paires(libres):
    """Scanner OTE pour Gold + paires Forex principales."""
    paires_ote = ["XAUUSD", "EURUSD", "GBPUSD", "USDJPY", "AUDUSD", "USDCAD"]
    for paire in paires_ote:
        statut, _ = est_symbole_autorise(paire)
        if statut == "BLOCAGE_TOTAL": continue
        cle = f"ote_{paire}"
        if cle in _ote_derniere_analyse and time.time()-_ote_derniere_analyse[cle] < 300:
            continue
        try:
            res = analyser_ote_complet(paire)
            if not res or not res.get("signal"): continue
            if res["rr"] < 1.5: continue
            _ote_derniere_analyse[cle] = time.time()

            # Mise en cache
            cle_cache = f"{paire}_OTE"
            action_txt = "🟢 ACHAT (CALL)" if res["tendance"]=="BULL" else "🔴 VENTE (PUT)"
            z = res["zone_ote"]
            signaux_cache[cle_cache] = {
                'time': time.time(), 'action': action_txt, 'conf': 95,
                'exp': "5 MIN", 'dur': 300, 'rsi': 50, 'stoch': 50,
                'bb': res["reaction_msg"], 'sc': float(res["score"]),
                'mt5_sl': z["sl"], 'mt5_tp': z["tp_1r"],
                'mt5_tp2': z["tp_15r"], 'mt5_rr': res["rr"]
            }

            # Notification
            signal_txt = formater_signal_ote(res, CAPITAL_ACTUEL)
            for uid in libres:
                pf = plateforme_trading.get(uid, "MT5")
                if pf == "POCKET" and paire not in FOREX_PAIRS: continue
                markup = InlineKeyboardMarkup().add(
                    InlineKeyboardButton(f"⚡ Frapper {paire[:3]}/{paire[3:]}", callback_data=f"set_{paire}")
                )
                try:
                    bot.send_message(uid, signal_txt, reply_markup=markup, parse_mode="Markdown")
                except: pass
        except Exception as e:
            print(f"[OTE] ⚠️ {paire}: {e}", flush=True)
